...
def is_valid_sequence(sequence):
    """
    Check if sequence only contains uppercase A, C, T, G.
...
def process_chunk_for_references(chunk, reference_ids, stats):
    ...
    stats['invalid_sequences'] += len(chunk[chunk['processid'].isin(reference_ids)]) - len(reference_matches)
...
def process_chunk_for_region(chunk, region, region_map, exclude_ids):
    ...
    filtered = chunk[
        ...
        (~chunk['processid'].isin(exclude_ids))
    ].copy()
...
def filter_bold(tree_file, infile, output_dir, logger):
    ...
    reference_fasta = output_dir / "reference_sequences.fasta"
...
def log_stats(stats, logger):
    ...
    logger.info(f"Invalid sequences (non ACTG): {stats['invalid_sequences']:,}")
